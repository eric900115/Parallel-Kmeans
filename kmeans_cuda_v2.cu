...
__global__ void KmeansAssignCluster(unsigned char* image_src, unsigned char* centroid, int* num_pt_cluster,\
    ...
    __shared__ unsigned char shared_sum_dist[num_cluster*3];
    __shared__ unsigned char shared_num_pt_cluster[num_cluster];
    ...
    for(int k = (threadIdx.x + threadIdx.y * blockDim.x); k < num_cluster; k += (blockDim.x * blockDim.y)) {
        shared_centroid[channels * k + 0] = centroid[channels * k + 0];
        shared_centroid[channels * k + 1] = centroid[channels * k + 1];
        shared_centroid[channels * k + 2] = centroid[channels * k + 2];
        shared_num_pt_cluster[k] = 0;
        shared_sum_dist[channels * k + 0] = 0;
        shared_sum_dist[channels * k + 1] = 0;
        shared_sum_dist[channels * k + 2] = 0;
    }
   ...
   __syncthreads();
    ...
    for(int k = 0; k < num_cluster; k++) {
        // calculate l2 norm
        dist = 0;
        dist += (img_src0 - shared_centroid[channels * k + 0]) * (img_src0 - shared_centroid[channels * k + 0]);
        dist += (img_src1 - shared_centroid[channels * k + 1]) * (img_src1 - shared_centroid[channels * k + 1]);
        dist += (img_src2 - shared_centroid[channels * k + 2]) * (img_src2 - shared_centroid[channels * k + 2]);
...
__global__ void KmeansWriteResult(unsigned char* image_src, unsigned char* image_result, \
   ...
   __syncthreads();
    ...
    image_result[channels * (idx_x + idx_y * width) + 0] = shared_centroid[channels * cluster_idx + 0];
    image_result[channels * (idx_x + idx_y * width) + 1] = shared_centroid[channels * cluster_idx + 1];
    image_result[channels * (idx_x + idx_y * width) + 2] = shared_centroid[channels * cluster_idx + 2];
}
