...
int main(int argc, char** argv) {
    ...
    for(int i = 0; i < height; i++) {
        for(int j = 0; j < width; j++) {
          image_result[channels * (j + i * width) + 0] = image_src[channels * (j + i * width) + 0];
          image_result[channels * (j + i * width) + 1] = image_src[channels * (j + i * width) + 1];
          image_result[channels * (j + i * width) + 2] = image_src[channels * (j + i * width) + 2];
        }
    }
